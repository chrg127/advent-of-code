...
(define (array- . args) (apply (curry map -) args))
(define (touching? h t) (andmap (lambda (x) (<= x 1)) (map abs (array- h t))))
(define ((clamp a b) x) (min b (max a x)))
(define (move-tail t h) (array+ t (map (clamp -1 1) (array- h t))))
...
(define (move-in-steps data move steps)
  (foldl (lambda (x r) (let ([rope (move-rope (car r) move)])
                         (list rope (set-add (cadr r) (car rope)))))
         data
         (range steps)))

(define (apply-instrs instrs rope-length)
  (foldl (lambda (x r) (move-in-steps r (make-move (car x)) (cadr x)))
