...
(define (radiuses x) (list (car x) (apply manhattan x)))
(define (parse input)
...
(define (inv-manhatthan x1 y1 y2 r)
...
(define ((xs-of y) sensor r)
  (inv-manhatthan (car sensor) (cadr sensor) y r))
...
(define (part2 input limit)
  (println (apply tuning-freq
                  (ormap (lambda (s)
                           (y-loop (car s) (cadr s) (find-point input limit)))
                         input))))

(time (define input1 (parse (file->lines "input15-1.txt")))
