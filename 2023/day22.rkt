...
(define (walk-graph supported supporting node)
  (define fallen (mutable-set node))
  (define (loop n)
      (when (andmap (lambda (x) (set-member? fallen x)) (hash-ref supported n))
        (set-add! fallen n)
        (for-each loop (hash-ref supporting n))))
  (for-each loop (hash-ref supporting node))
  (sub1 (set-count fallen)))

(define (solve2 in)
  (let ([g (make-graph (parse in))])
    (apply + (map (curry walk-graph (second g) (third g))
